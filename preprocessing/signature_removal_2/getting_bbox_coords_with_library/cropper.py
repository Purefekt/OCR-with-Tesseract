...
class Cropper:
    ...
    def find_contours(self, img):
        ...
        for c in cnt:
            ...
            if (h * w > self.min_region_size and h < copy_img.shape[0] and
                    w < copy_img.shape[1]):
    ...
    def crop_regions(self, mask, regions) -> dict:
        ...
        xmin = results[0]['cropped_region'][0]
        ymin = results[0]['cropped_region'][1]
        xmax = results[0]['cropped_region'][0] + results[0]['cropped_region'][2]
        ymax = results[0]['cropped_region'][1] + results[0]['cropped_region'][3]

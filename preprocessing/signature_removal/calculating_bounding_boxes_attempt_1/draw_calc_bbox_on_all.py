...
import json
"""
This script draws the bounding boxes using the calculated values from the calculated json file over the scanned images
with ground truth bounding boxes. Ground truth bounding boxes are in red and calculated bounding boxes are in blue.
"""
...
scanned_page_images_with_calc_bbox_dir = '/Users/veersingh/Desktop/docs_with_signs_dataset(tobacco800)/scanned_page_images_with_calc_bbox'
...
for filename in ground_truth_bbox.keys():
    ...
    image = scanned_page_images_with_ground_truth_bbox_dir + '/' + current_image_name
    image = cv2.imread(image)
    cv2.rectangle(image, (xmin, ymin), (xmax, ymax), (255, 0, 0), 5)
    cv2.imwrite(
        scanned_page_images_with_calc_bbox_dir + '/' + current_image_name,
        image)
